...
- **Changes**:
  - Added `ModuleInstance` to `src/engine/types.ts`: `{ id; canExecute(state): boolean; onRaidEnd(state, hook, context): void }` — a single synchronous listener, deliberately simpler than `BehaviorModule` (no interception, no async).
  - Added `src/engine/behaviors/hideoutModules.ts`: `RAID_END_MODULES: ModuleInstance[] = []` (empty registry) + `dispatchRaidEndModules(state, hook, context, modules = RAID_END_MODULES)` (modules param injectable for tests).
  - Wired `dispatchRaidEndModules(newState, hook, context)` into all 3 `AFTER_RAID_END` exits of `runRaidTickGenerator` (dehydration KIA, combat KIA, extraction) between the terminal `yield` and `return finishDraft(...)` — runs before settlement, so module-emitted intents land in the committed state.
  - `applyIntent` STASH_ADD now **upserts**: merges into an existing stash stack, or creates a new entry from the `ALL_ITEMS` template (previously a silent no-op for any item not already in the stash); unknown item ids now throw (`[engineContext] STASH_ADD: unknown item id`), matching the reducer's strictness for unknown target entities.
